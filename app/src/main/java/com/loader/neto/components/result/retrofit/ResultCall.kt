package com.loader.neto.components.result.retrofit

import okio.Timeout
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response
import com.loader.neto.components.result.HttpException
...
internal class ResultCall<T>(proxy: Call<T>) : CallDelegate<T, Result<T>>(proxy) {
    ...
    private class ResultCallback<T>(
        ...
        override fun onResponse(call: Call<T>, response: Response<T>) {
            val result: Result<T> = if (response.isSuccessful) Result.Success.HttpResponse(
                    value = response.body() as T,
                    statusCode = response.code(),
                    statusMessage = response.message(),
                    url = call.request().url.toString()
            ) else Result.Failure.HttpError(
                    HttpException(
                            statusCode = response.code(),
                            statusMessage = response.message(),
                            url = call.request().url.toString()
