...
sealed class Result<out T> {
    ...
    sealed class Success<T> : Result<T>() {
        ...
        ) : Success<T>(), com.loader.neto.components.result.HttpResponse
    ...
    sealed class Failure(open val error: Throwable? = null) : Result<Nothing>() {

        override fun toString() = "Failure($error)"

        class Error(override val error: Throwable) : Failure(error)

        class HttpError(override val error: HttpException) : Failure(), HttpResponse {
